...
收窄不是把类型“强行改小”，而是沿控制流积累证据。下面的路径从不可信输入开始，经过结构校验与判别字段，最终才能安全读取成功分支的数据。

<DocFigure
  src="/images/typescript/narrowing-control-flow.webp"
  alt="TypeScript 从 unknown 输入经过结构校验、判别联合和穷尽检查逐步收窄类型"
  caption="类型断言不会产生运行时证据；外部数据必须先校验，再让控制流完成收窄。"
  :width="1440"
  :height="900"
/>

分支结束后类型可能重新合并，因此每个可空值、联合成员和异常输入都要在真正使用字段之前被证明。
