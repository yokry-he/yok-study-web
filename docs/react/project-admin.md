...
后台项目不能只验收成功列表。加载、无结果、接口失败和无权限是四种不同状态，恢复方式和可暴露数据也不同。

<DocFigure
  src="/images/react/admin-states.webp"
  alt="React Admin 页面展示列表就绪、无结果、接口失败和 403 无权限四种状态"
  caption="错误、空态和无权限不能共用一个 fallback；每种状态都要有明确恢复路径。"
  :width="1440"
  :height="900"
/>

后续阶段会分别实现路由等待、列表空态、错误边界和权限边界，避免接口失败被误显示为“暂无数据”。
