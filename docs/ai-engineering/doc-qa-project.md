...
最终回答要把每个关键结论连接到可以打开的来源片段。证据不足时明确拒答，比生成一段没有依据的流畅文本更可靠。

<DocFigure
  src="/images/ai-engineering/doc-qa-citations.webp"
  alt="文档问答结果展示有据回答、两条具体来源和证据不足时的拒答状态"
  caption="引用必须包含文档、章节和版本，并在点击后回到原始上下文。"
  :width="1440"
  :height="900"
/>

来源权限发生变化时，检索缓存和答案缓存都要失效；否则旧引用仍可能把已经无权访问的内容暴露给用户。
