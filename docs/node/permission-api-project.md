...
权限 API 的最终响应不是简单角色名，而是资源动作、允许结果和数据范围。下面的结果既能驱动前端体验，也保留后端再次校验的明确权限码。

<DocFigure
  src="/images/node/permission-api-response.webp"
  alt="Node 权限接口报告列出 users read write、roles grant 和 audit read 的允许结果及部门数据范围"
  caption="前端可以消费权限结果，但后端业务接口仍必须按同一权限码和数据范围重新校验。"
  :width="1440"
  :height="900"
/>

客户端不需要得到完整策略表达式；返回稳定权限码和必要数据范围即可，避免暴露内部规则和敏感组织条件。
