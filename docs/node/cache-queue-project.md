...
上线后要把缓存命中、回源延迟、队列积压和失败任务放在同一张运行视图里，否则“Redis 在线”并不能证明业务健康。

<DocFigure
  src="/images/node/cache-queue-dashboard.webp"
  alt="Node 缓存队列运行看板展示缓存命中率、回源延迟、队列积压、失败任务和各队列状态"
  caption="扩容消费者之前先检查下游限流、任务幂等和失败原因，避免放大重试风暴。"
  :width="1440"
  :height="900"
/>

缓存降低读延迟，队列削峰和异步执行，两者的失败语义完全不同；后文会分别设计 key、TTL、retry、backoff 和死信处理。
