...
下面两张图使用完全相同的课程数据。桌面端把筛选和内容并排，移动端改为紧凑顶部操作与单列卡片；这不是把桌面页面整体缩小。

<DocFigure
  src="/images/frontend/responsive-desktop.webp"
  alt="桌面宽度的学习中心页面，顶部显示完整导航，左侧是课程筛选，右侧三列展示课程卡片。"
  caption="宽屏使用侧栏和三列卡片提高扫描效率，但内容 DOM 顺序仍应先主标题、筛选入口，再到课程列表。"
  :width="1440"
  :height="900"
/>

<DocFigure
  src="/images/frontend/responsive-mobile.webp"
  alt="390 像素宽度的学习中心页面，顶部导航收为紧凑操作，筛选变成按钮，课程卡片改为单列。"
  caption="窄屏保留最重要的品牌、继续学习和筛选入口；完整桌面导航不会堆到首屏上方。"
  :width="390"
  :height="844"
/>

响应式的核心是重新安排信息优先级和交互，不是只写一条 `width: 100%`。
