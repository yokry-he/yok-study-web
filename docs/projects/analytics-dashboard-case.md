...
一张可信看板必须同时回答“看了什么范围”“数据何时更新”“指标按什么口径计算”和“出现异常后去哪里追查”。

<DocFigure
  src="/images/projects/analytics-dashboard-overview.webp"
  alt="销售经营看板展示成交额、订单量、转化率、退款率、成交趋势、渠道排行、筛选范围和指标口径"
  caption="图表只是最后一层展示；筛选上下文、数据时效和指标口径决定数字能否用于决策。"
  :width="1440"
  :height="900"
/>

图中“成交额”明确扣除已完成退款，并说明跨日退款归属。实际项目应给指标口径单独版本号；口径修改后，缓存、导出和历史报表都要能识别版本。
