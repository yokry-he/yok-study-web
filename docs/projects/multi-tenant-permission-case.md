...
租户切换不是前端换一个菜单。一次请求解析出的租户上下文，必须继续进入 SQL、缓存 key、文件路径、消息任务与审计日志。

<DocFigure
  src="/images/projects/multi-tenant-permission-scope.webp"
  alt="多租户权限中心把用户会话、租户标识和权限码映射到 SQL 租户条件、缓存 key 与审计日志"
  caption="操作权限和数据范围必须一起校验；超级管理员也不能默认取消业务数据过滤。"
  :width="1440"
  :height="900"
/>

图中导出操作继承页面查询的数据范围。不要为导出任务另写一套无租户过滤的 SQL，否则普通页面看似安全，异步导出仍可能泄露其他租户数据。
