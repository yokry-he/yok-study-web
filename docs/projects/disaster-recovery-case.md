...
灾备切换必须把主备健康、复制延迟、流量方向、执行人和检查清单放在同一条受控流程中。

<DocFigure
  src="/images/projects/disaster-recovery-switch.webp"
  alt="灾备控制台展示上海主区域异常、北京备区域就绪、复制延迟 38 秒和七步切换检查清单"
  caption="切换不是一次 DNS 修改；每一步都要有门禁、证据、负责人和明确回退方式。"
  :width="1440"
  :height="900"
/>

图中的 RPO 为 38 秒，意味着业务必须知道可能丢失或延迟的写入范围。切换后还要验证登录、下单、支付回调和数据一致性，单个健康检查返回 200 不能证明恢复完成。
