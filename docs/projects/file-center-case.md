...
文件上传需要区分“正在传输”“已上传待扫描”和“已绑定业务可用”。只显示一个百分比无法表达完整生命周期。

<DocFigure
  src="/images/projects/file-center-task.webp"
  alt="企业文件中心展示合同分片上传进度、设备清单病毒扫描状态、图片业务绑定成功以及访问与生命周期规则"
  caption="对象存储返回成功后仍要确认元数据、扫描结果和业务绑定，附件才算真正可用。"
  :width="1440"
  :height="900"
/>

图中下载地址只在权限校验后短时签发，未绑定临时文件会被回收。这样既避免永久公开链接，也能清理用户上传后没有完成表单提交产生的无主对象。
