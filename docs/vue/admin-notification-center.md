...
消息中心要让未读状态在铃铛、列表和消息详情之间保持一致。图中同时展示了未读总数、消息类型、发生时间和业务跳转入口。

<DocFigure
  src="/images/vue/admin-notification-unread.webp"
  alt="Vue Admin 消息中心展示未读计数、消息分类、未读列表和批量已读操作"
  caption="未读数应由服务端记录和增量同步共同维护，不能只根据当前页列表长度计算。"
  :width="1440"
  :height="900"
/>
