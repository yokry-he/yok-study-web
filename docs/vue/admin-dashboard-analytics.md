...
先观察完整页面，再拆分实现。图中的筛选条件驱动指标、趋势和待办区，但每个区块仍有自己的加载、失败与刷新状态。

<DocFigure
  src="/images/vue/admin-dashboard.webp"
  alt="Vue Admin 工作台包含时间筛选、指标卡片、趋势图、排行榜和待办任务"
  caption="工作台是多个独立数据区块组成的任务入口，不应被实现成一个巨大的页面请求。"
  :width="1440"
  :height="900"
/>
