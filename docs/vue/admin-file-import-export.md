...
上传过程不能只有一个会转动的图标。下图把文件名、体积、阶段、百分比、速度、剩余时间和取消操作放在同一任务行中。

<DocFigure
  src="/images/vue/admin-file-upload-progress.webp"
  alt="Vue Admin 文件上传任务展示进度百分比、速度、剩余时间和取消按钮"
  caption="上传进度要绑定到具体文件任务；切换页面、重试和取消都应有明确状态迁移。"
  :width="1440"
  :height="900"
/>
