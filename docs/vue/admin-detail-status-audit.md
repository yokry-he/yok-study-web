...
详情页需要把“当前状态、业务信息、可执行动作、历史证据”放在同一条阅读路径上。下图将审计时间线放在主信息之后，方便先判断现状，再追溯原因。

<DocFigure
  src="/images/vue/admin-detail-audit.webp"
  alt="Vue Admin 订单详情页展示状态、客户信息、金额、操作按钮和审计时间线"
  caption="状态与操作记录必须来自同一业务对象；操作成功后重新拉取详情，避免页面局部状态互相矛盾。"
  :width="1440"
  :height="900"
/>
