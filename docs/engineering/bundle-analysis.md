...
包体积报告要把首屏总量、大模块、重复依赖和拆分动作放在一起。图中 charts vendor 与重复日期库已经指向两种不同修复方式。

<DocFigure
  src="/images/engineering/bundle-analysis.webp"
  alt="前端 Bundle 分析报告展示 612 KB 首屏 JavaScript、图表大模块、重复日期依赖和拆包建议"
  caption="压缩体积、解析执行成本、请求数量和缓存命中要一起评估，拆得越碎不一定越快。"
  :width="1440"
  :height="900"
/>

工具告诉你“什么占空间”，路由和用户路径决定“现在是否必须加载”；优化前后要在同一构建模式下复测。
